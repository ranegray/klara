...
Usage:
    ros2 run klara_hardware goto_pose stow
    ros2 run klara_hardware goto_pose stow --duration 5.0
    ros2 run klara_hardware goto_pose stow --pose-dir ~/.xle/poses
"""
...
from klara_hardware.joint_trajectory_guard_node import JOINT_LIMITS, LEFT_ARM_JOINTS
...
def load_pose(path: Path) -> dict:
    if not path.exists():
        raise SystemExit(
            ...
            f"`ros2 run klara_hardware capture_pose {path.stem}`."
        )
